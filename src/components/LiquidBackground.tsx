...
export default function LiquidBackground({ code = 0 }: Props) {
  ...
  return (
    <div className="fixed inset-0 -z-10 overflow-hidden bg-[#020617]">
      {/* 1. Primary Mesh Blobs - No grain, just pure gradients */}
      <motion.div
        animate={{ 
          x: [0, 50, -30, 0],
          y: [0, -40, 60, 0],
      ...
      {/* 2. Weather Particles (Using softer gradients to avoid pixelation) */}
        ...
        >
          ...
          {category === 'snow' && particles.map((p) => (
            <motion.div
              ...
              animate={{ 
                y: "110vh",
                x: [p.left, `calc(${p.left} + 20px)`, p.left]
              }}
              transition={{ 
